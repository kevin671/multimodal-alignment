...
from datasets.imagenet_CMC import RGB2Lab, RGB2YCbCr
...
def parse_option():
    ...
    opt.model_name = "calibrated_{}_bsz_{}_lr_{}_decay_{}".format(
        opt.model_name, opt.batch_size, opt.learning_rate, opt.weight_decay
    )
...
def set_optimizer(args, classifier):
    optimizer = optim.SGD(
        classifier.parameters(), lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay
    )
    return optimizer
        ...
        for idx, (input, target) in enumerate(val_loader):
            ...
            if idx % opt.print_freq == 0:
                print(
                    ...
                    "Acc@1 {top1.val:.3f} ({top1.avg:.3f})\t"
                    "Acc@5 {top5.val:.3f} ({top5.avg:.3f})".format(
                        idx, len(val_loader), batch_time=batch_time, loss=losses, top1=top1, top5=top5
                    )
                )
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        writer.add_scalar("train_acc", train_acc, epoch)  # logger.log_value("train_acc", train_acc, epoch)
        writer.add_scalar("train_acc5", train_acc5, epoch)  # logger.log_value("train_acc5", train_acc5, epoch)
        writer.add_scalar("train_loss", train_loss, epoch)  # logger.log_value("train_loss", train_loss, epoch)

        print("==> testing...")
        test_acc, test_acc5, test_loss = validate(val_loader, model, classifier, criterion, args)

        writer.add_scalar("test_acc", test_acc, epoch)  # logger.log_value("test_acc", test_acc, epoch)
        writer.add_scalar("test_acc5", test_acc5, epoch)  # logger.log_value("test_acc5", test_acc5, epoch)
        writer.add_scalar("test_loss", test_loss, epoch)  # logger.log_value("test_loss", test_loss, epoch)
