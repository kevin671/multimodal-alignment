...
from datasets.imagenet_CMC import ImageFolderInstance, RGB2Lab, RGB2YCbCr
from models.alexnet import MyAlexNetCMC
from models.resnet import MyResNetsCMC
from ssl_torch.NCEAverage import NCEAverage
from ssl_torch.NCECriterion import NCECriterion, NCESoftmaxLoss
...
def parse_option():
    ...
    opt.model_name = "memory_{}_{}_{}_lr_{}_decay_{}_bsz_{}".format(
        opt.method, opt.nce_k, opt.model, opt.learning_rate, opt.weight_decay, opt.batch_size
    )
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        time1 = time.time()
        l_loss, l_prob, ab_loss, ab_prob = train(
            epoch, train_loader, model, contrast, criterion_l, criterion_ab, optimizer, args
        )
        time2 = time.time()
        print("epoch {}, total time {:.2f}".format(epoch, time2 - time1))

        # tensorboard logger
        writer.add_scalar("Train/l_loss", l_loss, epoch)  # logger.log_value('l_loss', l_loss, epoch)
        writer.add_scalar("Train/l_prob", l_prob, epoch)  # logger.log_value('l_prob', l_prob, epoch)
        writer.add_scalar("Train/ab_loss", ab_loss, epoch)  # logger.log_value('ab_loss', ab_loss, epoch)
        writer.add_scalar("Train/ab_prob", ab_prob, epoch)  # logger.log_value('ab_prob', ab_prob, epoch)
